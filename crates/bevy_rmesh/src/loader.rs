...
use russimp::scene::{Scene as AssimpScene, PostProcess};
use std::path::Path;
...
impl AssetLoader for RMeshLoader {
    fn load<'a>(
        ...
        Box::pin(async move {
            Ok(load_rmesh(bytes, load_context).await?)
        })
    }
                    ...
                    if let Some(entity_type) = entity.entity_type {
                        match entity_type {
                            rmesh::EntityType::Light(data) => {
                                parent.spawn(PointLightBundle {
                                    ...
                                    point_light: PointLight {
                                        ...
                                        color: Color::rgb_u8(data.color.0[0], data.color.0[1], data.color.0[2]),
                                        ..Default::default()
                                    },
                                    ..Default::default()
                                });
                            ...
                            rmesh::EntityType::Model(data) => {
                                ...
                                parent.spawn(PbrBundle {
                                    transform: Transform {
                                        translation: (data.position[0] * ROOM_SCALE, data.position[1] * ROOM_SCALE, -data.position[2] * ROOM_SCALE).into(),
                                        ...
                                        scale: (data.scale[0] * ROOM_SCALE, data.scale[1] * ROOM_SCALE, data.scale[2] * ROOM_SCALE).into(),
                                    },
                                    mesh: load_context.get_handle(mesh_asset_path),
                                    material: load_context.get_handle(mat_asset_path),
                                    ..Default::default()
...
async fn load_xtexture<'a>(
    ...
    let uri = uri.as_ref();
    let parent = load_context.path().parent().unwrap();
    let image_path = parent.join("props").join(uri);
    let bytes = load_context.read_asset_bytes(image_path.clone()).await?;
...
async fn load_xfile<'a>(
    ...
    let uri = uri.as_ref();
    let parent = load_context.path().parent().unwrap();
    let xfile_path = parent.join("props").join(uri);
    let bytes = load_context.read_asset_bytes(xfile_path.clone()).await?;

    let scene = AssimpScene::from_buffer(&bytes,
    ...
    let mut result_mesh = Mesh::new(PrimitiveTopology::TriangleList);
    let positions: Vec<_> = mesh.vertices
        .iter()
        .map(|v| [v.x, v.y, v.z])
        .collect();
    let normals: Vec<_> = mesh.normals
        .iter()
        .map(|v| [v.x, v.y, v.z])
        .collect();
    let indices: Vec<_> = mesh.faces
        .iter()
    ...
    result_mesh.insert_attribute(Mesh::ATTRIBUTE_POSITION, positions);
    result_mesh.insert_attribute(Mesh::ATTRIBUTE_UV_0, tex_coords);
    result_mesh.insert_attribute(Mesh::ATTRIBUTE_NORMAL, normals);
    result_mesh.set_indices(Some(Indices::U32(indices)));
